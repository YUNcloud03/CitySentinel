...
export default function GreenCorridorPanel({ view, result, onResult }: Props) {
  ...
  return (
    <div className="green-corridor-panel">
      ...
      <p className="dim small">依主辦方路網、即時車速與官方號誌點位，計算救援路徑及號誌預控時序。</p>
      ...
      {result && (
        <div className="gc-result" aria-live="polite">
          ...
          <div className="gc-eta">
            ...
            <div className="gc-save">節省 {result.eta.saved_minutes} 分｜改善 {result.eta.improvement_pct}%</div>
          </div>
          ...
          {result.approval_status === "APPROVED_FOR_SIMULATION" && <div className="gc-runtime">
          ...
          <div className="gc-route" aria-label="救援路線">
            {result.route_details.map((road, index) => (
              <div key={road.segment_id}>
                <i>{index + 1}</i><span><b>{road.name}</b><small>{road.baseline_speed_kmh} → {road.corridor_speed_kmh} km/h｜{road.signal_count} 號誌</small></span>
              </div>
            ))}
          </div>
